...
* 对于一般的线性空间 $V, dimV = n$，考虑它的两个基 $\mathcal{B}$ 和 $\mathcal{C}$，那么对于 $V$ 中的任何一个 $\vec{b} \in V$，它在两个基下有不同的坐标，分别是 $\displaystyle \left[\vec{b}\right]_\mathcal{B}\in\mathbb{R^n}$ 和 $\displaystyle \left[\vec{b}\right]_\mathcal{C}\in\mathbb{R^n}$。从 $\displaystyle \left[\vec{b}\right]_\mathcal{B}\in\mathbb{R^n}$ 到 $\displaystyle \left[\vec{b}\right]_\mathcal{C}\in\mathbb{R^n}$，存在**唯一**的**坐标转移映射** $T,\ matrix\ P_\mathcal{C \leftarrow B} \in \mathbb{R^n}$，使得下式成立：
```math
