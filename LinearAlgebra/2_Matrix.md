...
### 可对角化的充要条件
矩阵可对角化的充要条件1缺乏操作性，仅仅是从对角化定义出发得到的。现在来思考下，如何找到矩阵 $A \in M_n(K)$ 的 $n$ 个线性无关的特征向量呢。  
假如说，矩阵 $A$ 有 $m$ 个不同的特征值，分别是 $\lambda_1,\ \lambda_2,\ \cdots,\ \lambda_m$。  
对于每一个特征值 $\lambda_i$，矩阵 $A$ 关于 $\lambda_i$ 的几何重数是 $r_i$，也就是说，$\lambda_i$ 可以贡献出 $r_i$ 个线性无关的特征向量。那么首先考虑，把这些**属于不同特征值的线性无关的特征向量**集合，取并集后是否依然线性无关呢？答案是是的，证明略。即：  
  
**属于不同特征值的、线性无关的特征向量集合，它们的并集仍然是线性无关的**  
  
所以，从 $m$ 个不同的特征值中，最多可得到 $r_1+r_2+\cdots+r_m$ 个线性无关的特征向量，所以若这个值等于 $n$，那么矩阵 $A$ 有 $n$ 个线性无关的特征向量，可对角化；如果这个值小于 $n$，那么矩阵 $A$ 不可对角化。  

#### 矩阵 $A$ 可对角化的充要条件2（从特征子空间的几何而来）：矩阵 $A$ 的不同特征值的几何重数之和等于 $n$  

这里可以立即给出一个矩阵 $A$ 可对角化的充分条件：  
**若 $A$ 有 $n$ 个不同的特征值，那么 $A$ 可对角化**  
...
3. 矩阵 $A$ 的不同特征值的几何重数之和 = 矩阵 $A$ 的不同特征值的代数重数之和 = $n$
4. 对于每一个不同的特征值 $\lambda_i$，矩阵 $A$ 关于 $\lambda_i$ 的几何重数 = 矩阵 $A$ 关于 $\lambda_i$ 的代数重数，且矩阵 $A$ 在数域 $K$ 上有 $n$ 个特征值（考虑重数）
5. 对于每一个不同的特征值 $\lambda_i$，矩阵 $A$ 关于 $\lambda_i$ 的几何重数 = 矩阵 $A$ 关于 $\lambda_i$ 的代数重数，且矩阵 $A$ 的特征多项式可以在数域 $K$ 上完全因式分解为一次因式的乘积。
6. 对于每一个不同的特征值 $\lambda_i$，矩阵 $A$ 关于 $\lambda_i$ 的几何重数 = 矩阵 $A$ 关于 $\lambda_i$ 的代数重数，且矩阵 $A$ 的特征多项式在复数域上的 $n$ 个根，都属于数域 $K$ 。  
