...
3. 「矩阵 $A$ 的不同特征值的代数重数之和 = $n$」不是「矩阵可对角化」的充分条件。举一个反例，考虑矩阵 $A$ 的特征多项式等于 $(\lambda - a)^n$，可见这里满足不同特征值的代数重数之和为 $n$。但是矩阵 $A$ 关于特征值 $a$ 的几何重数却可以是小于 $n$ 的
抽象地说，矩阵「可对角化」归根结底是一种几何性质，而特征多项式实质上作为行列式，在映射过程中损失了大量信息，所以特征多项式中蕴含的代数性质并不能推到是否「可对角化」。但是，同一个特征值的几何重数和代数重数之间，确实存在不等式关系，即几何重数小于等于代数重数。证明在后文
...
#### 矩阵 $A$ 可对角化的充要条件3（从特征多项式的代数而来）：矩阵 $A$ 的每个特征值的几何重数 等于 它的代数重数，且矩阵 $A$ 的特征多项式可以在数域 $K$ 上可完全因式分解为一次因式的乘积（即代数重数之和等于$n$）
