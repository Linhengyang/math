...
0. 首先，命题「矩阵 $A$ 的不同特征值的代数重数之和 = $n$」等价于命题「矩阵 $A$ 的特征多项式可完全因式分解为一次因式的乘积」等价于「矩阵 $A$ 有 $n$ 个特征值（考虑重数）」  
1. 显而易见，「矩阵 $A$ 的不同特征值的代数重数之和 = $n$」是「矩阵可对角化」的必要条件：由矩阵可对角化，推出矩阵跟某对角矩阵相似，相似的矩阵有相同的特征多项式。而对角矩阵的特征多项式是一次因式的乘积，即代数重数之和为 $n$
2. 「矩阵 $A$ 的不同特征值的代数重数之和 = $n$」不是「矩阵可对角化」的充分条件。举一个反例，考虑矩阵 $A$ 的特征多项式等于 $(\lambda - a)^n$，可见这里满足不同特征值的代数重数之和为 $n$。但是矩阵 $A$ 关于特征值 $a$ 的几何重数却可以是小于 $n$ 的
