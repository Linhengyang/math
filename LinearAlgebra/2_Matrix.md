...
所以，从 $m$ 个不同的特征值中，最多可得到 $r_1+r_2+\cdots+r_m$ 个线性无关的特征向量，所以若这个值大于等于 $n$，那么矩阵 $A$ 有 $n$ 个线性无关的特征向量，可对角化；如果这个值小于 $n$，那么矩阵 $A$ 不可对角化。实际上从下文可证得，这个和值不会大于 $n$。  

#### 矩阵 $A$ 可对角化的充要条件2（从特征子空间的几何而来）：矩阵 $A$ 的不同特征值的几何重数之和等于 $n$  

这里可以立即给出一个矩阵 $A$ 可对角化的充分条件：**若 $A$ 有 $n$ 个不同的特征值，那么 $A$ 可对角化**  
原因很简单，每个不同的特征值，至少能给出一个特征向量，加起来就给出了 $n$ 个线性不相关的特征向量
  

考虑一个附加问题：命题「矩阵 $A$ 的不同特征值的代数重数之和 = $n$」，和「矩阵可对角化」之间是什么关系？
1. 首先，命题「矩阵 $A$ 的不同特征值的代数重数之和 = $n$」等价于命题「矩阵 $A$ 的特征多项式可完全因式分解为一次因式的乘积」等价于「矩阵 $A$ 有 $n$ 个特征值（考虑重数）」  
2. 显而易见，「矩阵 $A$ 的不同特征值的代数重数之和 = $n$」是「矩阵可对角化」的必要条件：由矩阵可对角化，推出矩阵跟某对角矩阵相似，相似的矩阵有相同的特征多项式。而对角矩阵的特征多项式是一次因式的乘积，即代数重数之和为 $n$
3. 「矩阵 $A$ 的不同特征值的代数重数之和 = $n$」不是「矩阵可对角化」的充分条件。举一个反例，考虑矩阵 $A$ 的特征多项式等于 $(\lambda - a)^n$，可见这里满足不同特征值的代数重数之和为 $n$。但是矩阵 $A$ 的特征值 $a$ 的几何重数却可以是小于 $n$，因为这个特征空间的维度是由这个矩阵具体决定的。  

拔高一点说，矩阵「可对角化」归根结底是一种几何性质，而特征多项式实质上作为行列式，在矩阵到行列式的映射过程中损失了大量信息，所以特征多项式中蕴含的代数性质并不足以推导是否「可对角化」。不过，同一个特征值的几何重数和代数重数之间又确实存在不等式关系，即**几何重数小于等于代数重数**。证明在下文。  
...
那么有时候会想要得到一个“对不同的特征值逐一检查”的方法，使得若矩阵 $A$ 不可对角化，能提前发现而不是算完全部几何重数。考虑到
...
3. 矩阵 $A$ 的不同特征值的几何重数之和 = 矩阵 $A$ 的代数重数之和 = $n$
4. 对于每一个不同的特征值 $\lambda_i$，矩阵 $A$ 关于 $\lambda_i$ 的几何重数 = 矩阵 $A$ 关于 $\lambda_i$ 的代数重数，且矩阵 $A$ 在数域 $K$ 上不同特征值的代数重数之和等于 $n$
...
#### 矩阵 $A$ 可对角化的充要条件3（从特征多项式的代数而来）：矩阵 $A$ 的每个特征值的几何重数 等于 它的代数重数，且矩阵 $A$ 的特征多项式可以在数域 $K$ 上可完全因式分解为一次因式的乘积（即代数重数之和等于n）
